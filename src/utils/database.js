// Database adapter that switches between MySQL and PostgreSQL based on environment
...
if (DB_TYPE === 'postgres') {
                    ...
                    try {
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
                        ...
                        if (recordError.code === 'ER_DUP_ENTRY') {
                            // Handle duplicate by updating company data instead of skipping
                            console.log(`🔄 Duplicate detected for ${record.id || record.Id}, attempting company data update...`);
                            try {
                                ...
                                // MySQL2 returns [ResultSetHeader, FieldPacket[]] for execute()
                                const affectedRows = updateResult[0].affectedRows;
                                console.log(`✅ Update query executed: affected rows = ${affectedRows}`);
                                ...
                                if (affectedRows > 0) {
                                    ...
                                    console.log(`✅ Updated company data for duplicate record: ${record.id || record.Id}`);
                                } else {
                                    result.duplicatesSkipped++;
                                    result.duplicateIds.push(record.id || record.Id);
                                    console.warn(`⚠️  Duplicate entry skipped during batch insert: ${record.id || record.Id}`);
                                }
                            } catch (updateError) {
                                result.duplicatesSkipped++;
                                result.duplicateIds.push(record.id || record.Id);
                                console.error(`❌ Failed to update company data for duplicate ${record.id || record.Id}:`, updateError.message);
                            }
                        } else {
    ...
    `;
        ...
        /**
         * Get backup_table records with pagination for management
         */
        async getBackupRecordsWithPagination(limit = 50, offset = 0) {
            const sql = `
      SELECT Id, Phone, CompanyName, PhysicalAddress, Email, Website, source_file, created_at, updated_at
      FROM backup_table
    ...
    `;
        ...
        /**
         * Get specific backup record by ID
         */
        async getBackupRecordById(id) {
            const sql = `
      SELECT Id, Phone, CompanyName, PhysicalAddress, Email, Website, source_file, extracted_metadata, created_at, updated_at
      FROM backup_table
      WHERE Id = ?
    `;
