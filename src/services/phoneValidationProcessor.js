...
class PhoneValidationProcessor {
  ...
  async processBatch(batch) {
    ...
    for (const record of batch) {
      try {
        ...
        if (exists) {
          if (this.enableLogging) {
            console.log(`Record ${record.Id} already exists in check_table, skipping`);
          }
          continue;
        }
        ...
        // Extract company information from Excel metadata if available
        const companyInfo = this.extractCompanyInfoFromMetadata(record);
        
        // Insert into check_table with validation status and company fields
  ...
  async processSpecificRecords(recordIds) {
    try {
      ...
      for (const recordId of recordIds) {
        try {
          // Get record from backup_table with metadata
          const backupRecord = await databaseManager.query(
            'SELECT Id, Phone, source_file, extracted_metadata FROM backup_table WHERE Id = ?',
            [recordId]
          );
          ...
          // Validate and insert with company info from metadata
          const isValidSingaporePhone = singaporePhoneValidator.validateSingaporePhone(record.Phone);
          const companyInfo = this.extractCompanyInfoFromMetadata(record);
  ...
  /**
   * Extract company information from Excel metadata
